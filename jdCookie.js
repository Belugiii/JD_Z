...
// 判断环境变量里面是否有京东ck
if (process.env.Z_COOKIE) {
  if (process.env.Z_COOKIE.indexOf('&') > -1) {
    CookieJDs = process.env.Z_COOKIE.split('&');
  } else if (process.env.Z_COOKIE.indexOf('\n') > -1) {
    CookieJDs = process.env.Z_COOKIE.split('\n');
  } else {
    CookieJDs = [process.env.Z_COOKIE];
  }
}
