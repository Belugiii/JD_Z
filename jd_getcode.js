              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map((item) => {
                  if (item.taskType === 14) {
					...
					ddfactory.push(`${item.assistTaskDetailVo.taskToken}`);
                  }
                });
              }
            }
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (data.factoryList && data.productionList) {
				  ...
				  jxfactory.push(`${data.user.encryptPin}`);
                }
              } else {
...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
	  ...
	  farm.push(`${$.farmInfo.farmUserPro.shareCode}`);
